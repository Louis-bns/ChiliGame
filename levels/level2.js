(function () {
  window.LEVEL2 = {
    ...
    // Kuh-Konfig wird auf "K" Tile gerendert
    cowConfig: {
      enabled: true,
      sprite: "assets/Cow.png",
      spritePatched: "assets/Cow_pflaster.png", // Kuh mit Pflaster (Wenn gebraucht)
    ...
    flags: {
      ...
      hackCollected: false, // damit Hack nur einmal eingesammelt
    },
    ...
    checkTriggers(playerTx, playerTy, ctx) {
      ...
      // N soll nach H  NICHT mehr triggern
    ...
    interactions: {
      ...
      M: (ctx) => {
        if (phase === 1)
          return msg(
            ctx.showTempMessage,
            "The machine appears to be demaged  ..."
          );
        if (phase === 2)
          return msg(
            ctx.showTempMessage,
            "Something is wrong with the fuse boxes."
          );
        if (phase === 3) return msg(ctx.showTempMessage, "Repaired and ready");
        if (phase === 4 || phase === 5)
          return msg(ctx.showTempMessage, "Repaired and ready");
        return;
      },
      ...
      C: (ctx) => {
        ...
        if (phase === 5) {
          ...
          // Schwarzblende + Phase6 + Kuh-Sprite swap (ggf)
      ...
      // Container (H)  ab Phase 6 "voll mit hack"
      H: (ctx) => {
        if (phase < 6) {
  ...
  function msg(showTempMessage, text, duration, extra = {}) {
    // lock erlaubt auch: msg(fn, "Text", { lockPlayer: true })
    ...
    showTempMessage(text, finalDuration, {
      ...
      center: true,
      typewriter: true,
      charDelay, // nicht hart 40 überschreiben
      ...extra,
    });
  }
      ...
      if (!itemHeld) {
        if (pendingPickup !== tile) {
          ...
          if (tile === "I")
            return msg(
              ctx.showTempMessage,
              "Rubber boots?!...take them along?",
              { lockPlayer: true }
            );
          if (tile === "2")
            return msg(ctx.showTempMessage, "Hay?!...take them along?", {
              lockPlayer: true,
            });
          if (tile === "3")
            return msg(ctx.showTempMessage, "Carrots?!...take them along?", {
              lockPlayer: true,
            });
          return;
        } else {
          ...
          if (tile === "I")
            return msg(ctx.showTempMessage, "Taken rubber boots");
  ...
  function handleCowInteract(ctx) {
    ...
    if (phase === 1) {
      msg(ctx.showTempMessage, "Moo... I'm ready, but the machine's broken.", {
        lockPlayer: true,
      });
      phase = 2;
      resetFuseSequence();
    ...
    if (phase === 4) {
      ...
      if (itemHeld === "hay") {
        msg(ctx.showTempMessage, "That looks delicious .......", {
          lockPlayer: true,
        });
